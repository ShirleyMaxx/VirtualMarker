...
from utils.coord_utils import cam2pixel, process_bbox, scale_bbox, _box_to_center_scale, _center_scale_to_box, rigid_align, get_bbox, get_intrinsic_metrix
from utils.aug_utils import j2d_processing, augm_params, j3d_processing, juvd_processing, flip_joints_3d
from utils.preprocessing import load_img, im_to_torch, generate_integral_uvd_target
from utils.vis import vis_joints_3d, render_mesh
from utils.funcs_utils import save_obj
...
class JointsDataset(torch.utils.data.Dataset):
    ...
    def compute_per_mesh_err_dict(self, pred_mesh, gt_mesh, pred_pose_reg, gt_pose_reg, gt_pose_root=None, focal_l=None, center_pt=None):
        ...
        if cfg.test.save_obj:
            print(f"==> save mesh in obj format to {cfg.vis_dir}")
            for pred, gt, datum, root_cam, focal, center in tzip(pred_mesh, gt_mesh, self.datalist, gt_pose_root, focal_l, center_pt):
                if 'PW3D' in cfg.dataset.test_list:
                    filename = datum['img_name'].split('/')[-2] + f"{datum['annot_id']:08d}"
                else:
