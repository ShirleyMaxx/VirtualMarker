...
from .base import prepare_network
from dataset.multiple_datasets import MultipleDatasets
from core.config import cfg
from utils.funcs_utils import lr_check, save_obj
from utils.vis import vis_joints_3d, render_mesh, denormalize_image
from utils.preprocessing import load_img
from utils.aug_utils import get_affine_transform, flip_img, augm_params
from utils.coord_utils import _box_to_center_scale, pixel2cam
...
class Simple3DMeshTrainer:
    ...
    def train(self, epoch, n_iters_total, master):
        ...
        for i, meta in enumerate(batch_generator):
            ...
            _, pred_uvd_pose, adaptive_A, confidence, pred_mesh, _ = self.model(imgs, inv_trans, intrinsic_param, gt_pose_root, depth_factor, flip_item=None, flip_mask=flip_mask)
        ...
        for dataset_name, val_dataset, val_loader in zip(cfg.dataset.test_list, self.val_dataset, self.val_loader_list):
            ...
            with torch.no_grad():
                for i, meta in enumerate(loader):
                    ...
                    _, pred_uvd_pose, _, confidence, pred_mesh, _ = self.model(imgs, inv_trans, intrinsic_param, gt_pose_root, depth_factor, flip_mask=None, is_train=False)
                    ...
                    _, _, _, _, pred_mesh_flip, _ = self.model(imgs_flip, inv_trans, intrinsic_param, gt_pose_root, depth_factor, flip_item=(pred_uvd_pose, confidence), flip_output=True, flip_mask=None, is_train=False)
                ...
                if master:
                    ...
                    mesh_flip_error_dict = val_dataset.compute_per_mesh_err_dict(results['pred_mesh_flip'], results['gt_mesh'],\
                        results['pred_pose_flip'], results['gt_pose'], results['gt_pose_root'], results['focal_l'], results['center_pt'])
        ...
        for dataset_name, val_dataset, val_loader in zip(cfg.dataset.test_list, self.val_dataset, self.val_loader_list):
            ...
            with torch.no_grad():
                for i, meta in enumerate(loader):
                    ...
                    _, pred_uvd_pose, _, confidence, pred_mesh, pred_mesh_post = self.model(imgs, inv_trans, intrinsic_param, gt_pose_root, depth_factor, flip_mask=None, is_train=False)
                    ...
                    _, _, _, _, pred_mesh_flip, pred_mesh_post_flip = self.model(imgs_flip, inv_trans, intrinsic_param, gt_pose_root, depth_factor, flip_item=(pred_uvd_pose, confidence), flip_output=True, flip_mask=None, is_train=False)
